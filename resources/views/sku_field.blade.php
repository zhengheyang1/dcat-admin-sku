<div class="{{$viewClass['form-group']}}">
    <label for="{{$formId}}" class="{{$viewClass['label']}} control-label">{{$label}}</label>

    <div class="{{$viewClass['field']}}">
        {{--        <textarea id="{{$formId}}" name="{{$ name}}">{{ old($column, $value) }}</textarea>--}}

        <div class="sku_warp {{$class}}">
            <input type="hidden" class="Js_sku_input" name="{{$name}}" value="{{old($column, $value)}}">
            <div class="sku_attr_select">
                <span class="btn Js_single_btn" data-type="single">单规格</span>
                &nbsp;&nbsp;&nbsp;&nbsp;
